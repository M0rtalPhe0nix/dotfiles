...
- "Where in this repository should I create the skill source?" If the user has no preference, propose a repository-local `skills/` development directory and explain that installation belongs to the capability workflow.
...
Before running `init_skill.py`, ask where in the current repository the user wants the skill source created. If they do not specify a location, propose `./skills` and confirm it. Never default to a global discovery directory; repository installation belongs to `dotfiles skills add` after the source has portable capability metadata.
