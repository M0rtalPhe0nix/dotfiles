...
```bash
python3 scripts/create_tactical_roster.py - --name brazil-2026 --web-link
```
...
```bash
python3 scripts/create_building_elevation.py - --name empire-state --web-link
```
...
```bash
python3 scripts/generate_excalidraw.py spec.json output.excalidraw
```

For a Claude-style handoff with a visible preview and browser page, run:

```bash
python3 scripts/generate_excalidraw.py spec.json output.excalidraw --preview
```

For the default web-link handoff, pass the spec through stdin:

```bash
python3 scripts/create_web_diagram.py - --name checkout-flow
```
...
```bash
python3 scripts/session_state.py save \
...
```bash
python3 scripts/create_web_diagram.py spec.json --name checkout-flow --no-web-link
```
