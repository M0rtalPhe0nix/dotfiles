...
sh "$root/tests/validate-ai-artifacts.sh" --json \
	--root "$root/tests/fixtures/ai-artifacts/valid" \
	--catalog "$root/tests/fixtures/capability-catalog/valid" |
	jq -e '.status == "PASS" and (.errors | length == 0)' >/dev/null

if sh "$root/tests/validate-ai-artifacts.sh" \
	--root "$root/tests/fixtures/ai-artifacts/valid" \
	--catalog "$root/tests/fixtures/capability-catalog/invalid" >/dev/null 2>&1; then
	printf '%s\n' "invalid capability catalog unexpectedly passed." >&2
	exit 1
fi
